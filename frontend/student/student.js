...
async function loadDashboardStats() {
  ...
  const avg = results.length
    ? (results.reduce((a, b) => a + b.score, 0) / results.length).toFixed(2)
    : 0;
...
async function loadAvailableTests() {
  ...
  if (!tests.length) {
    container.innerHTML = '<p class="empty-state">No tests available for your class right now.</p>';
    return;
  }
  container.innerHTML = tests.map(t => `
    <div class="test-card">
      ...
      <div class="test-meta">
        ...
        <span>📊 +${t.marks.correct} / ${t.marks.wrong} / ${t.marks.skip}</span>
      </div>
      <div class="test-meta">
        <span>📅 ${new Date(t.startTime).toLocaleString()}</span>
        <span>⏰ ${new Date(t.endTime).toLocaleString()}</span>
      </div>
    ...
    optionsHtml = `
      <div class="numerical-input">
        <input type="number" step="any" value="${answers[q.questionId] || ''}" 
          onchange="saveAnswer('${q.questionId}', this.value)" placeholder="Enter your answer">
      </div>
    `;
  }
    ...
    <div class="test-layout">
      <div class="test-main">
        <div class="test-header">
          ...
          <div class="timer ${timeLeft < 60 ? 'warning' : ''}">⏱️ <span id="timerDisplay">${formatTime(timeLeft)}</span></div>
          <button class="btn-outline" onclick="toggleLanguage()">🌐 ${lang === 'en' ? 'हिंदी' : 'English'}</button>
        </div>
        <div class="question-container">
          ${images}
          <div class="question-text"><strong>Q${currentQIndex+1}:</strong> ${questionText}</div>
          <div class="options-container">${optionsHtml}</div>
        </div>
        <div class="question-actions">
          <button class="btn-outline" onclick="flagQuestion('${q.questionId}')">
            ${flaggedQuestions.has(q.questionId) ? '🏁 Unflag' : '🚩 Flag'}
          </button>
          <button class="btn-outline" onclick="clearAnswer('${q.questionId}')">🗑️ Clear</button>
        </div>
        <div class="navigation">
          <button class="btn-outline" onclick="navigate(-1)" ${currentQIndex === 0 ? 'disabled' : ''}>← Previous</button>
          <span>${currentQIndex+1} / ${testQuestions.length}</span>
          <button class="btn-primary" onclick="navigate(1)">
            ${currentQIndex === testQuestions.length-1 ? 'Submit Test' : 'Next →'}
          </button>
        </div>
      </div>
      <div class="test-palette">
        ...
        <div class="palette-grid">
          ${testQuestions.map((q, i) => `
            <button class="palette-btn ${getButtonClass(q.questionId)}" onclick="jumpTo(${i})">${i+1}</button>
          `).join('')}
        </div>
        <div class="palette-legend">
          <span><span class="legend-dot answered"></span> Answered</span>
          <span><span class="legend-dot flagged"></span> Flagged</span>
          <span><span class="legend-dot not-answered"></span> Not Answered</span>
        </div>
      </div>
    </div>
  `;
...
window.flagQuestion = (qId) => {
  flaggedQuestions.has(qId) ? flaggedQuestions.delete(qId) : flaggedQuestions.add(qId);
  renderTestInterface();
};

window.clearAnswer = (qId) => {
  delete answers[qId];
  renderTestInterface();
};
...
window.navigate = (dir) => {
  if (dir === 1 && currentQIndex === testQuestions.length - 1) {
    if (confirm('Submit the test? You cannot change answers after submission.')) {
      submitTest();
    }
    return;
  }
...
function startTimer() {
  timerInterval = setInterval(async () => {
    ...
    if (data.paused) {
      document.getElementById('testInterface').innerHTML = `
        <div class="paused-overlay">
          <h2>⏸️ Test Paused by Admin</h2>
          <p>Please wait. The test will resume shortly.</p>
        </div>
      `;
      return;
    }
    ...
    if (timerEl) {
      ...
      timerEl.parentElement.classList.toggle('warning', timeLeft < 60);
    }
    if (timeLeft <= 0) {
...
function handleVisibilityChange() {
  if (document.hidden) {
    ...
    if (visibilityWarningCount >= MAX_WARNINGS) {
      alert('You have switched tabs too many times. The test will be auto-submitted.');
      submitTest();
    } else {
...
async function loadResults() {
  ...
  tbody.innerHTML = results.map(r => `
    <tr>
      <td>${testMap[r.testId] || r.testId}</td>
      <td>${r.score}</td>
      <td>${r.rank}</td>
...
window.viewAnalysis = async (resultId) => {
  ...
  let html = `<h3>Test Analysis</h3><div style="max-height:400px;overflow-y:auto;">`;
  result.answers.forEach(ans => {
    ...
    html += `
      <div style="border-bottom:1px solid #eee;padding:12px 0;">
        ...
        <p>Marks: ${ans.marksAwarded} (${ans.isCorrect ? '✓' : '✗'})</p>
      </div>
    `;
  });
  html += '</div><button class="btn-primary" onclick="closeModal()">Close</button>';
  showModal(html);
};
...
async function loadDiscussions() {
  ...
  if (!testId) {
    document.getElementById('discussionsList').innerHTML = '<p class="empty-state">Select a test</p>';
    return;
  }
...
async function loadMessages() {
  ...
  const warningEl = document.getElementById('messageWarning');
  const inputEl = document.getElementById('newMessage');
  if (studentData?.status === 'blocked') {
    warningEl.textContent = 'Your account is blocked. You cannot send messages.';
